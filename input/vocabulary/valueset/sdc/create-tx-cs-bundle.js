...
async function createFHIRTransactionBundle() {
  try {
    ...
    // Filter for ValueSet files matching the pattern
    const valueSetFiles = files.filter(file => 
      file.startsWith('ValueSet-') && file.endsWith('.json')
    );
    ...
    // Process each ValueSet file
    for (const filename of valueSetFiles) {
