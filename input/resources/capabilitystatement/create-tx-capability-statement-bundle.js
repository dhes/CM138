...
async function createFHIRTransactionBundle() {
  try {
    // Directory containing CapabilityStatement JSON files
    const inputDir = path.join(__dirname);
    
    // Read all files in the directory
    const files = await fs.readdir(inputDir);
    
    // Filter for CapabilityStatement files matching the pattern
    const capabilityStatementFiles = files.filter(file => 
      file.startsWith('CapabilityStatement-') && file.endsWith('.json')
    );
    ...
    // Process each CapabilityStatement file
    for (const filename of capabilityStatementFiles) {
      ...
      const entry = {
        request: {
          method: 'PUT',
          url: `CapabilityStatement/${resource.id}`
        },
        resource: resource
      };
