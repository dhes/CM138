...
async function createFHIRTransactionBundle() {
  try {
    // Directory containing CodeSystem JSON files
    const inputDir = path.join(__dirname);
    
    // Read all files in the directory
    const files = await fs.readdir(inputDir);
    
    // Filter for CodeSystem files matching the pattern
    const codeSystemFiles = files.filter(file => 
      file.startsWith('CodeSystem-') && file.endsWith('.json')
    );
    ...
    // Process each CodeSystem file
    for (const filename of codeSystemFiles) {
